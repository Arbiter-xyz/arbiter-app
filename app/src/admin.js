...
async function fetchAdmin(path) {
  ...
  if (res.status === 401) {
    try {
      ...
      // Storage may be blocked; still bounce to login.
    }
...
document.getElementById('btn-admin-login').addEventListener('click', () => {
  ...
  try {
    ...
    showLogin('Could not save the admin token in this browser (storage may be blocked). Try again with storage enabled.');
    return;
  }
  showShell();
  selectView('overview');
